...
// دالة مساعدة لتحديد ما إذا كان اللون داكناً لحساب التباين التلقائي
...
// دالة تفعيل وتبديل الوضع الليلي العام وحفظه في التخزين المحلي
function setSiteTheme(mode) {
  ...
  // حاسم: إعادة تطبيق متغيرات الألوان فوراً بعد تبديل الوضع، حتى لا تبقى
  // ألوان القالب المحفوظ (خلفية/نص) "عالقة" فوق الوضع الجديد وتُسبب اختفاء
  // العناصر (نص داكن على خلفية داكنة أو العكس)
...
// الاستماع لحدث تبديل اللغة وإعادة رسم العناصر الحية فوراً
...
// دالة ذكية لتطبيق متغيرات ألوان الهوية مع حماية تلقائية للتباين بين النص والخلفية.
// المشكلة التي تحلها: عند تفعيل "قالب ليلي" (أو عند تبديل المستخدم للوضع الليلي يدوياً
// بينما هوية المطعم المحفوظة فاتحة)، كانت ألوان الخلفية/البطاقة/النص المحفوظة في القالب
// تُفرض دائماً كـ inline style على <html> و<body>، وهذا يتغلب في الأولوية على قواعد
// [data-theme="dark"] في style.css ويكسر تزاوج (خلفية داكنة + نص فاتح)، فتظهر عناصر
// بنص داكن فوق خلفية داكنة (أو العكس) وتختفي تماماً.
// الحل: نطبّق لون الخلفية/البطاقة/النص القادم من القالب فقط إذا كانت درجته (فاتح/داكن)
// متوافقة فعلياً مع الوضع الحالي المُفعّل على الموقع، وإلا نُحرّر (remove) المتغير
// بالكامل من الـ inline style ليعمل محرك الوضع الليلي المدمج في style.css بألوانه
// الآمنة والمتزاوجة تلقائياً بدلاً منه.
function applyThemeColorVariables(s) {
  ...
  // إدارة وسوم القوالب بذكاء: لو القالب محدد نطبقه، لو مخصص نفكه فوراً لمنع التعارض
  if (s.theme.preset) {
  ...
  // ألوان الهوية (البراند): آمنة فوق أي خلفية فاتحة أو داكنة، تُطبَّق دائماً
  ...
  if (s.theme.darkColor) {
    // لون داكن بالتصميم دائماً (أزرار ملكية، فوتر...)، لا علاقة له بوضع الموقع العام
    setVar("--char", s.theme.darkColor);
    setVar("--brand-dark", s.theme.darkColor);
    setBootstrapRgb("--bs-dark-rgb", s.theme.darkColor);
  }

  // خلفية الموقع + خلفية البطاقات + لون النص: ثلاثتهم "زوج" يجب أن يبقى متجانساً
  // (خلفية فاتحة مع نص داكن) أو (خلفية داكنة مع نص فاتح)، فنطبّق قيمة القالب فقط
  // عندما تكون درجتها اللونية متوافقة فعلياً مع الوضع الحالي المُفعّل
...
function applyDynamicThemeAndContent(s) {
  ...
  const defaultRestaurantName = isEn ? "Restaurant" : "المطعم";
  ...
  const defaultFooter = isEn
    ...
    : `جميع الحقوق محفوظة © 2026 ${restaurantName}`;

  const replaceRestaurantName = (value) =>
    String(value || "").replace(
      /مطعم أبو قورة الفلاحي|مطبخ أبو قورة الفلاحي|مطبخ أبو قورة|أبو قورة|Abu Qorah Restaurant|Abu Qorah Kitchen|Abu Qorah|المطعم|Restaurant/g,
      restaurantName,
    );
  ...
  if (s.content) {
    ...
    document.querySelectorAll("[data-restaurant-logo]").forEach((el) => {
      el.alt = isEn ? `${restaurantName} Logo` : `شعار ${restaurantName}`;
    });
...
// المعاينة الحية الفورية عبر الرسائل مع لوحة الإدارة
  ...
  try {
    ...
    if (document.getElementById("contactWhatsappLink")) {
      ...
      const defaultText = isEn
        ...
        : "السلام عليكم، أريد الاستفسار عن المنيو والطلبات";
      ...
      const whatsappMsg = encodeURIComponent(
        isEn
          ...
          : `*طلب جديد من ${restaurantName}*\n` +
              ...
              (extraPhone ? `*رقم إضافي:* ${extraPhone}\n` : "") +
              `*نوع الطلب:* ${orderType.toUpperCase()}\n` +
              (tableNumber ? `*رقم الطاولة:* ${tableNumber}\n` : "") +
              (orderType === "delivery"
              ...
              itemsSummary +
              `\n-------------------------\n` +
              `*المبلغ الإجمالي:* ${orderObj.totalPrice} ${t("egp")}`,
      );
      ...
      <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden bg-white">
        <div class="p-3 pb-0 d-flex justify-content-between align-items-center">
          <span class="badge bg-light text-dark border px-3 py-1.5 rounded-pill fw-bold fs-7">
            <i class="fa-solid fa-crown me-1 text-warning"></i><span data-restaurant-name>${escapeHTML((window.getRestaurantName && window.getRestaurantName()) || (isEn ? "Restaurant" : "المطعم"))}</span>
          </span>
  ...
  if (!user) {
    drawerUserSection.innerHTML = `
      <div class="drawer-guest-box p-3 rounded-4 bg-light border text-center">
        ...
        <h6 class="fw-bold text-dark mb-1" data-i18n="user_guest_welcome">${t("user_guest_welcome", {}, "أهلاً بك يا ضيفنا العزيز")}</h6>
        <p class="small text-muted mb-3" data-i18n="user_guest_prompt">${t("user_guest_prompt", {}, "سجل الدخول لمتابعة طلباتك والاستمتاع بتجربة أسرع")}</p>
...
function renderUserSessionUI(user) {
  ...
  // 1. للشاشات الكبيرة والمكتبية (Desktop Header)
  if (userHeaderArea) {
    if (isAdmin) {
...
// حالة وإدارة الترقيم المتزايد والتحميل التدريجي (Infinite Scroll & Server-side Pagination)
...
async function loadProductsFromDB(options = {}) {
  ...
  if (!isAppend) {
    ...
    if (container.children.length === 0 || options.reset) {
      container.innerHTML = `
        <div class="col-md-6 col-lg-4"><div class="food-card p-3 placeholder-glow border rounded-4"><div class="bg-secondary rounded-3 mb-3" style="height: 180px; opacity: 0.12;"></div><div class="placeholder col-8 mb-2"></div><div class="placeholder col-5"></div></div></div>
        <div class="col-md-6 col-lg-4"><div class="food-card p-3 placeholder-glow border rounded-4"><div class="bg-secondary rounded-3 mb-3" style="height: 180px; opacity: 0.12;"></div><div class="placeholder col-8 mb-2"></div><div class="placeholder col-5"></div></div></div>
        <div class="col-md-6 col-lg-4"><div class="food-card p-3 placeholder-glow border rounded-4"><div class="bg-secondary rounded-3 mb-3" style="height: 180px; opacity: 0.12;"></div><div class="placeholder col-8 mb-2"></div><div class="placeholder col-5"></div></div></div>
      `;
    }
  ...
  if (productPaginationState.isLoading || !productPaginationState.hasMore)
    return;
  ...
  try {
    ...
    if (!isAppend) {
      ...
      renderFoodCards(newProducts, container, false);
      if (!isHomePage) {
        setupProductInfiniteScroll();
      }
    } else {
      allProductsFromDB = allProductsFromDB.concat(newProducts);
      renderFoodCards(newProducts, container, true);
    }
    ...
    if (hasMore) {
      ...
      if (endOfListEl && allProductsFromDB.length > pageSize) {
    ...
    if (!isAppend) {
      container.innerHTML = `<div class="col-12 text-center py-5"><p class="text-muted fs-5">${t("error_loading_dishes")}</p></div>`;
    }
  } finally {
    productPaginationState.isLoading = false;
    productPaginationState.abortController = null;
    if (!productPaginationState.hasMore && sentinel) {
...
// دالة تبديل رؤية كلمة المرور عند الضغط على زر العين
